...
import { Edit2, FileText, Loader2, RefreshCw, Save, Trash2, X } from 'lucide-vue-next'
...
async function removeDocument(document: LegalDocument) {
  ...
  try {
    ...
    if (editingDocument.value?.id === document.id) resetForm()
  ...
  <div class="space-y-4">
    <section class="rounded-[1rem] border border-[color:var(--border-soft)] bg-[var(--surface-primary)] p-4">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p class="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-[var(--text-tertiary)]">Legal</p>
    ...
    <section class="grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <form class="rounded-[1rem] border border-[color:var(--border-soft)] bg-[var(--surface-primary)] p-4" @submit.prevent="saveDocument">
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-start gap-3">
            ...
            <div>
              <h2 class="font-display text-base font-semibold text-[var(--text-primary)]">{{ editingDocument ? 'Edit document' : 'Create document' }}</h2>
              <p class="mt-1 text-sm text-[var(--text-secondary)]">Published documents are visible to users.</p>
            </div>
          </div>
          <button v-if="editingDocument" type="button" class="grid h-9 w-9 place-items-center rounded-[0.75rem] text-[var(--text-secondary)] hover:bg-[var(--surface-muted)]" aria-label="Clear form" @click="resetForm">
            <X class="h-4 w-4" />
          </button>
        </div>
        ...
        <div class="mt-5 grid gap-4 md:grid-cols-2">
          <label>
            <span class="mb-2 block text-sm font-semibold text-[var(--text-primary)]">Document</span>
            <select v-model="form.slug" class="h-11 w-full rounded-[0.85rem] border border-[color:var(--border-soft)] bg-[var(--surface-secondary)] px-3 text-sm outline-none focus:border-[var(--accent)]" :disabled="!!editingDocument">
              <option v-for="document in allowedDocuments" :key="document.slug" :value="document.slug">{{ document.title }}</option>
            </select>
          </label>
          ...
          <label class="md:col-span-2">
            ...
            <textarea v-model="form.content" class="min-h-72 w-full resize-y rounded-[0.85rem] border border-[color:var(--border-soft)] bg-[var(--surface-secondary)] px-3 py-3 font-mono text-sm outline-none focus:border-[var(--accent)]" placeholder="<p>Policy content here</p>"></textarea>
          </label>
        </div>
        ...
        <div class="mt-5 flex justify-end gap-2">
          <button type="button" class="inline-flex h-11 items-center justify-center rounded-[0.85rem] border border-[color:var(--border-soft)] px-4 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--accent-strong)]" @click="resetForm">Clear</button>
          ...
          <article v-for="document in allowedDocuments" :key="document.slug" class="p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div class="min-w-0">
                ...
                <p class="mt-1 break-all text-xs text-[var(--text-tertiary)]">{{ document.slug }} / {{ document.publicPath }}</p>
              ...
              <div class="flex shrink-0 gap-2">
                <button
                  ...
                  @click="editDocument(documentBySlug.get(document.slug)!)"
                >
                  <Edit2 class="h-4 w-4" />
                </button>
                <button
